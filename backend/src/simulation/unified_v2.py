...
#: Target revenue convergence per tick toward the steady state. Linear
#: convergence rather than ODE — "obvious math" baseline.
REVENUE_CONVERGENCE_RATE: float = 0.05
...
class CompanyAgentV2:
    ...
    async def step(self) -> TickResult:
        ...
        # Effective capacity = base * location count, with infra multiplier.
        infra_mult = 1.0 + INFRA_TO_CAPACITY_GAIN * sum(bridge_agg.infrastructure.values())
        total_capacity = (
        ...
        # 5. Revenue and satisfaction
        # Quality multiplier from bridge.quality bucket.
        ...
        # Marketing pressure from bridge.marketing bucket
        marketing_pressure = 1.0 + MARKETING_TO_DEMAND_GAIN * sum(
            bridge_agg.marketing.values()
        )
        # Demand pulled from market (set by MultiCompanySim) or from intrinsic
        # marketing pressure if running solo. Apply demand-side shock multiplier.
        demand_mult = float(env.get("market_demand_mult", 1.0))
        if self.allocated_demand > 0:
            effective_demand = self.allocated_demand * marketing_pressure * demand_mult
        else:
            # Solo-mode fallback: tam * marketing_pressure / competitor_density
            base = self.seed.tam / max(1, self.seed.competitor_density + 1)
            effective_demand = (
                base * marketing_pressure * demand_mult * self.satisfaction / 100.0
            )

        # Served customers bounded by capacity
        served = min(effective_demand, total_capacity)
        ...
        # Bankruptcy check — single-tick for simplicity. v1 has multi-tick;
        # can be re-added later.
        if self.cash < -daily_burn * 30:  # 30 days of negative-burn = dead
            self.alive = False
            return self._build_dead_tick_result()
    ...
    def _apply_decision(self, decision: CeoDecision) -> None:
        ...
        # adjust_params: stored as side-channel for now; pure math doesn't
        # consume them yet. Wave 5 wires the LLM tier's price/marketing
        # tweaks into the math layer. Logged for transcript / critic agent.
        if decision.adjust_params:
            log.debug(
                "Tick %d: %s adjust_params=%s (not yet wired into math)",
                self.tick, decision.tier, decision.adjust_params,
            )
...
class MultiCompanySimV2:
    ...
    Each tick:
      ...
      2. Compute each company's quality/marketing attractiveness.
      ...
      7. (Optional) new entrant spawn — TODO future.
    """
    ...
    def __init__(
        ...
        *,
        ...
        max_ticks: int,
        tam_initial: float,
        market_growth_rate: float = 0.0001,
        share_alpha: float = 1.0,  # marketing exponent
        share_beta: float = 1.5,   # quality exponent
    ) -> None:
        self.sim_id = sim_id
    ...
    def is_complete(self) -> bool:
        return self.tick >= self.max_ticks or not self.alive_companies
    ...
    async def step(self) -> dict[str, Any]:
        ...
        if not alive:
            return {"tick": self.tick, "tam": self.tam, "alive": 0, "results": []}

        # Compute per-company "quality" and "marketing" magnitudes from the
        # bridge aggregate. These drive multinomial logit share attraction.
