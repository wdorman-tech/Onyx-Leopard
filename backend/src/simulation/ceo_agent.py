...
async def call_ceo_agent(
    ...
    """Call Claude API for a CEO decision. Falls back to defaults on failure."""
    client = _get_client()
    ...
    log.error("CEO agent failed all retries for %s, using defaults", company_name)
    return CEODecision(reasoning="Maintaining current course (agent unavailable)")
...
def apply_decision(company: CompanyAgent, decision: CEODecision, tick: int) -> None:
    ...
    company._marketing_boost = decision.marketing_intensity
    company._expansion_pace = decision.expansion_pace
    company._max_locations_per_year = decision.max_locations_per_year
